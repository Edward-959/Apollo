...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, para):
    ...
    ############################################
    start_date_minus_lag_2 = Dtk.get_n_days_off(start_date, -(para + 2))[0]
    ...
    # 计算因子值
    data_ret = data_close.diff(para) / data_close.shift(para)
    factor_data = data_ret * data_volume
...
def main():
    ...
    para = 6
    factor_name = "F_D_GTJA29"
    file_name = factor_name
    factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, para)
