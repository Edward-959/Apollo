...
class FactorMinIntraDistanceToMaK(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            ...
            # 计算日内的MA_k
            intraday_ma_df = stock_minute_data.rolling(self.__k).mean()
            # 计算每分钟收盘价到日内MA_k的距离
            stock_minute_data[code] = stock_minute_data['close'] / intraday_ma_df['close'] - 1
...
def main():
    ...
    factor_generator = FactorMinIntraDistanceToMaK(codes=stock_code_list, start_date_int=istart_date_int,
