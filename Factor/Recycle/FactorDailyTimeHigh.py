...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, n):
    ...
    # 计算因子值
    data_high = data_high * data_volume / data_volume
    factor_data = data_high.rolling(n, min_periods=1).apply(lambda x: pd.Series(x).idxmax(), raw=True)
    factor_data = -factor_data.sub(n-1)
    factor_data = factor_data * data_volume / data_volume
...
def main():
    ...
    for n in [20]:
        factor_name = "F_D_TimeHigh_" + str(n)
