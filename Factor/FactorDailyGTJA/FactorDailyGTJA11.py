...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag):
    ...
    # 计算因子值
    factor_data = (data_close - data_low - data_high + data_close) / (data_high - data_low) * data_volume
    factor_data = factor_data.fillna(method='ffill').rolling(lag).sum()
...
def main():
    ...
    for lag in [6, 12, 18]:
        factor_name = "F_D_GTJA11"+'Lag'+str(lag)
