# -*- coding: utf-8 -*-
# @Time    : 2018/11/19 8:55
# @Author  : 011673
# @File    : FactorDailyGTJA40.py
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, k):
    ...
    volume_plus = copy.deepcopy(volume)
    volume_minus = copy.deepcopy(volume)
    volume_plus[condition == False] = 0
    volume_plus = volume_plus.rolling(window=26, min_periods=1).sum()
    volume_minus[condition == True] = 0
    volume_minus = volume_minus.rolling(window=26, min_periods=1).sum()
    factor_data = 100 * volume_plus / volume_minus
...
def main():
    ...
    for i_lag in [3, 5]:
        k = i_lag
        factor_name = "F_D_GTJA40" + '_' + str(k)
