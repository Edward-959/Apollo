...
class NonFactorDailyMinUpPreCloseVol(DailyMinFactorBase):
    def __init__(self, alpha_factor_root_path, stock_list, start_date_int, end_date_int, params):
        ...
        self.__data_pre_close = Dtk.get_panel_daily_pv_df(stock_list, start_date_int, end_date_int, pv_type='pre_close', adj_type='NONE')
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            stock_day_preclose = self.__data_pre_close.loc[:, code]
            stock_minute_chg = stock_minute_close.div(stock_day_preclose, axis=0) - 1
            stock_minute_upclose = stock_minute_vol[stock_minute_close.sub(stock_day_preclose, axis=0) > 0]
            stock_upclose_vol = (stock_minute_chg * stock_minute_upclose).sum(axis=1)
            factor_data = stock_upclose_vol * self.__data_volume.loc[:, code] / self.__data_volume.loc[:, code]
