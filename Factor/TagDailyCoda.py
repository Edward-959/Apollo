...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, ma_k):
    ...
    # 获取数据
    data_df_coda = Dtk.get_panel_daily_pv_df(stock_list, start_date, end_date_plus_k_2, pv_type='twp_coda',
                                             adj_type='FORWARD')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    tag_coda = data_df_coda.shift(-ma_k) / data_df_coda - 1
    # 保留start_date至end_date（前闭后闭）期间的数据
    factor_data = tag_coda.loc[start_date: end_date].copy()
    ########################################
    # 因子计算逻辑到此为止，以下勿随意变更 #
    ########################################
