...
根据该分钟收盘价是否涨停确定当前分钟是否能够买入成交
将非涨停分钟的成交量加总得到当日可买入总成交量
...
class DataDailyMinBuyableVolume(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            stock_twap_all_day = (stock_minute_data_volume * if_not_limit_up).sum(axis=1)
