...
在买入过程中，如果当前分钟K线的收盘价等于涨停价，则无法成交，能够实现的成交比例相应减少
"""
...
class DataDailyMinBuyTwap(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            limit_up_price = np.round(pre_close * 1.1, 2)
            stock_minute_data_close = stock_minute_data['close'].unstack()
            # 如果分钟K线收盘价等于涨停价，为0，否则为1
            if_not_limit_up = np.sign(np.round(stock_minute_data_close.sub(limit_up_price, axis=0), 2)).clip(upper=0).replace(-1, 1)
            stock_twap_all_day = (stock_minute_data_close * if_not_limit_up).sum(axis=1) / if_not_limit_up.sum(axis=1)
