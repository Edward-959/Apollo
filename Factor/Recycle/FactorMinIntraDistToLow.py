...
"""
@author: 006688
计算最近lag根K线的最低价(包含当前K线)
如果lag=242，则计算当日开盘到当前K线为止(包含当前K线)的日内最低价
"""
...
class FactorMinIntraDistToLow(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            ...
            stock_minute_data[code] = stock_minute_data['close'] / stock_minute_intra_low.stack() - 1
...
def main():
    ...
    for lag in [30, 242]:
        factor_name = "F_M_IntraDistToLow" + str(lag)  # 这个因子名可以加各种后缀，用于和相近的因子做区分
        file_name = factor_name
        factor_generator = FactorMinIntraDistToLow(codes=stock_code_list, start_date_int=istart_date_int,
