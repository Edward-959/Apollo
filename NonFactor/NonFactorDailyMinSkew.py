...
class NonFactorDailyMinSkew(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            ret_log = 100 * pd.DataFrame(np.log(ret.values), index=ret.index, columns=ret.columns)
            ret_log_square = ret_log * ret_log
            rvar = ret_log_square.sum()
            rvar = rvar * rvar * rvar
            rvar = pd.Series(np.sqrt(rvar.values), index=rvar.index)
            temp: pd.DataFrame = ret_log * ret_log * ret_log
            temp = math.sqrt(242) * temp.sum()
            factor_data = temp / rvar
