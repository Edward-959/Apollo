...
Position = (Close - D_Low) / (D_High - D_Low)
"""
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag):
    ...
    # 计算因子值
    factor_high = data_high.rolling(lag).max()
    factor_low = data_low.rolling(lag).min()
    factor_data = (data_close - factor_low) / (factor_high - factor_low)
...
def main():
    ...
    for lag in [3,5,10,20]:
        factor_name = "F_D_PositionHL" + str(lag)
