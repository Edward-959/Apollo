...
class TagMinHighGrowth242(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            stock_minute_high: pd.DataFrame = stock_minute_data.drop(['open', 'close', 'low', 'volume', 'amt'], axis=1)
            stock_minute_close: pd.DataFrame = stock_minute_data.drop(['open', 'high', 'low', 'volume', 'amt'], axis=1)
            # 【因子计算过程】
            stock_minute_rolling_high = stock_minute_high.rolling(window=242).max()
            stock_minute_data[code] = stock_minute_rolling_high['high'].shift(-242) / stock_minute_close['close'] - 1
...
def main():
    ...
    factor_name = "TagMinHighGrowth242"  # 这个因子名可以加各种后缀，用于和相近的因子做区分
    file_name = factor_name
    factor_generator = TagMinHighGrowth242(codes=stock_code_list, start_date_int=istart_date_int,
