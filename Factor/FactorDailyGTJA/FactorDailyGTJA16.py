...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag, rank_universe):
    ...
    universe = Dtk.get_panel_daily_info(stock_list, start_date_minus_lag_2, end_date, rank_universe)
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    data_volume_rank = data_volume[universe == 1].rank(1).fillna(method='ffill')
    data_vwap = data_amt / data_volume
    data_vwap = data_vwap * adj_df  # 计算后复权的vwap
    data_vwap_rank = data_vwap[universe == 1].rank(1).fillna(method='ffill')
    corr_data_volume_rank_and_data_vwap_rank = data_volume_rank.rolling(lag).corr(data_vwap_rank)
    corr_data_volume_rank_and_data_vwap_rank_rank = corr_data_volume_rank_and_data_vwap_rank[universe == 1].rank(1)

    factor_data = -corr_data_volume_rank_and_data_vwap_rank_rank.rolling(lag).max()
...
def main():
    ...
    for lag in [5, 10, 15]:
        factor_name = "F_D_GTJA16"+'Lag'+str(lag)
        file_name = factor_name
        rank_universe = 'alpha_universe'
        factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, lag, rank_universe)
