...
收盘前15 分钟成交量占日总成交量的比例（不包括集合竞价成交量）
"""
from Factor.DailyMinFactorBase import DailyMinFactorBase
import DataAPI.DataToolkit as Dtk


class NonFactorDailyMinCloseVolumePercent(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            stock_volume_daily = stock_minute_volume.sum(axis=1)
            stock_volume_close = stock_minute_volume.loc[:, 1445:1459].sum(axis=1)
            factor_data = (stock_volume_close / stock_volume_daily).to_frame(code)
