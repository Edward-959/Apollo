...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag):
    ...
    # 计算因子值
    data_last_close = data_close.shift(1)
    data_tr = np.maximum(data_high - data_low, np.maximum(
        abs(data_high - data_last_close), abs(data_low - data_last_close)))
    data_atr = data_tr.rolling(lag).mean()
    factor_data = data_atr / data_close
