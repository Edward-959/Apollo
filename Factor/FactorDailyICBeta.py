...
class FactorDailyICBeta(DailyFactorBase):
    ...
    def factor_calc(self):
        ...
        cashflow_df = Dtk.return_statement_type_filtered_df("AShareCashFlow", self.x,
                                                           last_report_date0, self.end_date)
        x = cashflow_df[self.x]
        x = Dtk.df_unstack_and_filter(x, self.stock_list, report_dates_list)

        data_df = x.copy()
        data_result1 = pd.DataFrame(index=report_dates_list, columns=self.stock_list)
        for i_report_date in report_dates_list[4:]:
            if str(i_report_date)[4:8] == '1231':
                data_result1.loc[i_report_date] = data_df.loc[i_report_date]
            else:
                data_result1.loc[i_report_date] = data_df.loc[i_report_date] + data_df.loc[
                    int(str(i_report_date - 10000)[0:4] + '1231')] - data_df.loc[i_report_date - 10000]
        x_ttm = data_result1

        x_ttm = (x_ttm - x_ttm.mean()) / x_ttm.std()
        y_ttm = (y_ttm - y_ttm.mean()) / y_ttm.std()
        factor_dict = {}
        # 对全部股票做遍历
        for column in list(x_ttm.columns):
            x_column = list(x_ttm[column])
            ...
            for i in range(len(x_column)):
                if i < self.rolling_window:
                    ...
                    factor_dict[column].append(k)

        factor_dict = pd.DataFrame(factor_dict, index=x_ttm.index)
        factor_dict = factor_dict.reindex(columns=x_ttm.columns)
