...
日内分钟收益率与成交量的相关性，并求n日移动加权平均
"""
from Factor.DailyMinFactorBase import DailyMinFactorBase
import DataAPI.DataToolkit as Dtk


class NonFactorDailyMinCorrRetVolume(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            stock_minute_volume = stock_minute_data['volume'].unstack().loc[:, 930:1456]
            stock_minute_close = stock_minute_data['close'].unstack()
            stock_minute_ret = (stock_minute_close.diff(axis=1) / stock_minute_close.shift(1, axis=1)).loc[:, 930:1456]
            stock_corr = stock_minute_ret.corrwith(stock_minute_volume, axis=1)
