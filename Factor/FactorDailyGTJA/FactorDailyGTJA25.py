...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, para1, para2, para3, para4):
    linear_seq = np.linspace(1, para3, para3)
    alpha_universe = Dtk.get_panel_daily_info(stock_list, start_date, end_date, info_type="alpha_universe")
    ...
    ############################################
    start_date_minus_lag_2 = Dtk.get_n_days_off(start_date, -(para4 + 2))[0]
    ...
    data_pct_chg: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_lag_2, end_date,
                                                           pv_type='pct_chg', adj_type='NONE')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    data_close_delta = data_close.diff(para1)
    data_volume_mean = data_volume / data_volume.rolling(para2).mean()
    data_volume_mean = data_volume_mean.rolling(para3).apply(lambda x: sum(np.array(x)*linear_seq)/sum(linear_seq), raw=True)
    data_volume_mean_rank = data_volume_mean[alpha_universe == 1].rank(axis=1)
    data_pct_chg_sum = data_pct_chg.rolling(para4).sum()
    factor_data = -1 * (data_close_delta[alpha_universe == 1] * (1-data_volume_mean_rank)).rank(axis=1) * (
            1+data_pct_chg_sum[alpha_universe == 1].rank(axis=1))
...
def main():
    ...
    para3 = 9
    para4 = 250
    factor_name = "F_D_GTJA25"
    file_name = factor_name
    factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, para1, para2, para3, para4)
