...
class NonFactorDailyMinSignAmplitude(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            stock_minute_high_max = stock_minute_high.max(axis=1)
            stock_minute_low_min = stock_minute_low.min(axis=1)
            stock_sign_amp = (stock_minute_high_max - stock_minute_low_min) / (
                        stock_minute_high_max + stock_minute_low_min) * np.sign(
                (stock_minute_high_time - stock_minute_low_time).fillna(0))
            factor_data = stock_sign_amp.to_frame(code)
