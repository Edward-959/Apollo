...
Amt Magnification -- Amt_Ema_FastLag / Amt_Ema_SlowLag
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, ema_fast_lag,
                     ema_slow_lag):
    ...
    ############################################
    start_date_minus_slow_lag_2 = Dtk.get_n_days_off(start_date, -(ema_slow_lag + 2))[0]
    ...
    # 计算因子值
    factor_ema_slow = data_df.ewm(span=ema_slow_lag, adjust=False).mean()
    factor_ema_fast = data_df.ewm(span=ema_fast_lag, adjust=False).mean()
    factor_data = factor_ema_fast / factor_ema_slow
...
def main():
    ...
    fast_lag = 3
    slow_lag = 6
    factor_name = "F_D_AmtMagniFS" + '_' + str(fast_lag) + '_' + str(slow_lag)
    file_name = factor_name
    factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, fast_lag, slow_lag)
