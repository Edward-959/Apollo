...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name):
    ...
    ret_df = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date, pv_type='pct_chg')
    ...
    # 计算因子值
    temp_df1 = ret_df.rolling(lag_k).std()
    ans_part1_df = temp_df1.copy()
    ans_part1_df[ret_df >= 0] = close_df
    ans_part2_df = ans_part1_df ** 2
    ans_part2_df[ans_part2_df < 5] = 5
    ans_df = ans_part2_df[alpha_universe == 1].rank()
