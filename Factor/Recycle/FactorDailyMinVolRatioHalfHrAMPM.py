...
class FactorDailyMinVolRatioHalfAMPM(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:  # 如可正常取到行情DataFrame
            ...
            stock_minute_volume_1st_half_hr_am = stock_minute_volume.loc[:, 900: 930].sum(axis=1)
            stock_minute_volume_1st_half_hr_pm = stock_minute_volume.loc[:, 1300: 1330].sum(axis=1)
            factor_value = stock_minute_volume_1st_half_hr_am / stock_minute_volume_1st_half_hr_pm
            factor_data = pd.DataFrame(factor_value, index=list(stock_minute_volume.index), columns=[code])
        ########################################
        # 因子计算逻辑到此为止，以下勿随意变更 #
        ########################################
...
def main():
    ...
    factor_name = "F_D_MinVolRatioHalfHrAMPM"   # 这个因子名可以加各种后缀，用于和相近的因子做区分
    file_name = factor_name
    factor_generator = FactorDailyMinVolRatioHalfAMPM(codes=stock_code_list, start_date_int=istart_date_int,
