...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag):
    ...
    ############################################
    linear_seq = np.linspace(1, lag, lag)
    start_date_minus_lag_2 = Dtk.get_n_days_off(start_date, -(2 * lag + 2))[0]
    ...
    # 计算因子值
    data_close_mean = data_close.rolling(lag).mean()
    data_reg_beta = data_close_mean.rolling(lag).apply(lambda x: stats.linregress(np.array(x), linear_seq).slope, raw=True)
    factor_data = data_reg_beta
...
def main():
    ...
    para1 = 6
    factor_name = "F_D_GTJA21"
    file_name = factor_name
    factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, para1)
