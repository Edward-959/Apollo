...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, k):
    ...
    delta:pd.DataFrame=vwap-vwap.shift(3)
    delta=delta.rolling(window=5,min_periods=1).max()
    rank_value=delta.rank(axis=1)
    factor_data=-rank_value
