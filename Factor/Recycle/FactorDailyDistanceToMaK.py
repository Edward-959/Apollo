...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, ma_k):
    ...
    data_df: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date,
                                                      pv_type='close', adj_type='FORWARD')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    # 计算MA_k
    ma_k_df = data_df.rolling(ma_k).mean()
    factor_data = data_df / ma_k_df - 1
...
def main():
    ...
    for lag in [3, 5, 10, 20]:
        factor_name = "F_D_DistToMa" + str(lag)
