...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, k):
    ...
    signal_1: pd.DataFrame = (open_.rolling(window=5).sum()) * (ret.rolling(window=5).sum())
    signal_1 = signal_1 - signal_1.shift(10)
    signal_1 = signal_1.rank(axis=1)
    factor_data = -signal_1
...
def main():
    ...
    for i_lag in [3, 5]:
        k = i_lag
        factor_name = "F_D_GTJA37" + '_' + str(k)
