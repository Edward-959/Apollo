...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag1, lag2, rank_universe):
    ...
    ############################################
    start_date_minus_lag_2 = Dtk.get_n_days_off(start_date, -(max(lag1, lag2) + 2))[0]
    ...
    factor_data = data_vwap - data_vwap.rolling(lag1).max()
    factor_data = factor_data[universe == 1].rank(1)
    delta = data_close - data_close.shift(lag2)
    factor_data = factor_data**delta
...
def main():
    ...
    for lag1 in [5, 10, 15]:
        lag2 = 5
        factor_name = "F_D_GTJA17"+'Lag'+str(lag1)+'Lag'+str(lag2)
        file_name = factor_name
        rank_universe = 'alpha_universe'
        factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, lag1, lag2, rank_universe)
