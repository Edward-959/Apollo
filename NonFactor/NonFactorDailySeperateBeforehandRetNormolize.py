...
class NonFactorDailySeperateBeforehandRetNormolize(DailyFactorBase):
    ...
    def factor_calc(self):
        valid_date = Dtk.get_n_days_off(self.start_date, -2 * self.n - 100)[0]
        close: pd.DataFrame = Dtk.get_panel_daily_pv_df(self.stock_list, valid_date, self.end_date,
                                                        pv_type='close', adj_type='FORWARD')
        turn: pd.DataFrame = Dtk.get_panel_daily_info(self.stock_list, valid_date, self.end_date, info_type='turn')
