# -*- coding: utf-8 -*-
# @Time    : 2018/11/15 15:18
# @Author  : 011673
# @File    : FactorDailyGTJA36.py
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, k):
    ...
    vwap: pd.DataFrame = amt / volume
    vwap_rank = vwap.rank(axis=1)
    volume_rank: pd.DataFrame = volume.rank(axis=1)
    cor = volume_rank.rolling(window=6).corr(vwap_rank)
    sum_: pd.DataFrame = cor.rolling(window=2).sum()
    factor_data = sum_.rank(axis=1)
...
def main():
    ...
    for i_lag in [3, 5]:
        k = i_lag
        factor_name = "F_D_GTJA36" + '_' + str(k)
