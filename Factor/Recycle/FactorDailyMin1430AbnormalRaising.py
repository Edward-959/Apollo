...
class FactorDailyMin1430AbnormalRaising(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:  # 如可正常取到行情DataFrame
            ...
            amplify = stock_minute_high.loc[:, 925:1430].max(axis=1) / stock_minute_low.loc[:, 925:1430].min(axis=1) - 1

            raising = stock_minute_close.loc[:, 1500] / stock_minute_close.loc[:, 1430] - 1

            factor_data = pd.DataFrame(raising / amplify, index=list(stock_minute_close.index), columns=[code])
        ########################################
        # 因子计算逻辑到此为止，以下勿随意变更 #
        ########################################
...
def main():
    ...
    factor_name = "F_D_Min1430AbnormalRaising"   # 这个因子名可以加各种后缀，用于和相近的因子做区分
