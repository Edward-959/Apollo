...
def decay_linear(df: pd.DataFrame, n):
    def find_value(data):
        if len(data) == 1:
            return data[0]
        else:
            return len(data) * data[-1] + find_value(data[:-1])

    return df.rolling(window=n, min_periods=1).apply(find_value) / ((1 + n) * n / 2)

def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, k):
    ############################################
    ...
    start_date_minus_k_2 = Dtk.get_n_days_off(start_date, -(k + 2))[0]
    ...
    volume: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date,
                                                     pv_type='volume', adj_type='FORWARD')
    amt: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date,
                                                  pv_type='amt', adj_type='FORWARD')
    open_: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date,
                                                    pv_type='open', adj_type='FORWARD')

    vwap = amt / volume
    new_price:pd.DataFrame=0.3*vwap+0.7*open_
    volume_mean:pd.DataFrame=volume.rolling(window=180,min_periods=1).mean()
    sum_volume_mean=volume_mean.rolling(window=37,min_periods=1).sum()
    cor=new_price.rolling(window=14,min_periods=1).corr(sum_volume_mean)
    decay:pd.DataFrame=decay_linear(cor,12)
    decay_rank_1=decay.rank(axis=1)
    close_delay=close-close.shift(2)
    decay_close_delta:pd.DataFrame=decay_linear(close_delay,8)
    decay_close_delta=decay_close_delta.rank(axis=1)
    factor_data=-(decay_close_delta-decay_rank_1)
...
def main():
    ...
    for i_lag in [3, 5]:
        k = i_lag
        factor_name = "F_D_GTJA39" + '_' + str(k)
