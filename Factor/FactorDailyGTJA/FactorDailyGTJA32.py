# -*- coding: utf-8 -*-
# @Time    : 2018/11/15 12:53
# @Author  : 011673
# @File    : FactorDailyGTJA32.py
# -*- coding: utf-8 -*-
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, k):
    ...
    high: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date,
    ...
    corr:pd.DataFrame=high_rank.rolling(window=3).corr(volume_rank)
    corr_rank=corr.rank(axis=1)
    sum_cor_rank=corr_rank.rolling(window=3).sum()
    factor_data=-sum_cor_rank
...
def main():
    ...
    for i_lag in [3, 5]:
        k = i_lag
        factor_name = "F_D_GTJA32" + '_' + str(k)
