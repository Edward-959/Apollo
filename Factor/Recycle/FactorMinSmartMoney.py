...
"""
@author: 006688
聪明钱在交易过程中，往往呈现“单笔订单数量更大，订单报价更为激进”的特征
用以下指标S来衡量每一分钟交易的“聪明程度”：
St = abs(Rt)/sqrt(Vt)
其中，Rt 为第t分钟的涨跌幅，Vt 为第t分钟的成交量。指标 St 的值越大，则表示该分钟的交易越“聪明”
在实际计算中，将Vt替换成了成交额
"""
...
class FactorMinSmartMoney(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            stock_minute_data[code] = (stock_minute_data['close'] / stock_minute_data['open'] - 1) / np.sqrt(
                stock_minute_data['amt'])
...
def main():
    ...
    factor_name = "F_M_SmartMoney"  # 这个因子名可以加各种后缀，用于和相近的因子做区分
    file_name = factor_name
    factor_generator = FactorMinSmartMoney(codes=stock_code_list, start_date_int=istart_date_int,
