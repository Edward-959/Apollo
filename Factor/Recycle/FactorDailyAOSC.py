...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, fast_lag, mid_lag,
                     slow_lag):
    ...
    # 计算因子值
    amt_ma_fast_lag = data_df.rolling(fast_lag).mean()
    amt_ma_mid_lag = data_df.rolling(mid_lag).mean()
    amt_ma_slow_lag = data_df.rolling(slow_lag).mean()
    factor_data = (amt_ma_mid_lag - amt_ma_slow_lag) / amt_ma_fast_lag
...
def main():
    ...
    for f_lag, m_lag, s_lag in zip([2, 3], [4, 6], [8, 9]):
        factor_name = "F_D_AOSC" + '_' + str(f_lag) + '_' + str(m_lag) + '_' + str(s_lag)
        file_name = factor_name
        factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, f_lag, m_lag, s_lag)
