...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, para1, para2, para3):
    log_seq = np.logspace(para3, 1, para3, base=0.9)
    ...
    ############################################
    start_date_minus_lag_2 = Dtk.get_n_days_off(start_date, -(para2 + para3 + 2))[0]
    ...
    # 计算因子值
    data_ret1 = data_close.diff(para1) / data_close.shift(para1) * 100
    data_ret2 = data_close.diff(para2) / data_close.shift(para2) * 100
    data_ret_sum = data_ret1 + data_ret2
    factor_data = data_ret_sum.rolling(para3).apply(lambda x: sum(np.array(x) * log_seq) / sum(log_seq), raw=True)
