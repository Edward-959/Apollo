...
"""
@author: 006688
计算当前K线前lag分钟的涨跌幅，计算使用的是K线的收盘价
其中lag为参数，且不涉及到前一日的数据
不足lag分钟，则从开盘开始计算
需要计算的lag在main中进行设置
"""
...
class FactorMinIntradayGrowth(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            ...
            stock_minute_growth = stock_minute_data['close'].unstack()
            for i in range(1, len(complete_minute_list)):
                start_minute = complete_minute_list[max(0, i-self.__lag)]
                stock_minute_growth[complete_minute_list[i]] = stock_minute_growth[complete_minute_list[i]] / \
                    stock_minute_growth[start_minute] - 1
            stock_minute_growth[925] = 0
            stock_minute_data[code] = stock_minute_growth.stack()
...
def main():
    ...
    for lag in [5, 10, 15, 30, 242]:  # 需要计算的参数，242表示开盘到当前K线的涨跌幅
        factor_name = "F_M_IntradayGrowth" + str(lag)  # 这个因子名可以加各种后缀，用于和相近的因子做区分
        file_name = factor_name
        factor_generator = FactorMinIntradayGrowth(codes=stock_code_list, start_date_int=istart_date_int,
