...
class FactorDailyUpDownVolSub(DailyFactorBase):
    def __init__(self, alpha_factor_root_path, stock_list, start_date_int, end_date_int, params):
        super().__init__(alpha_factor_root_path)
        self.stock_list = stock_list
        self.start_date = start_date_int
        self.end_date = end_date_int
        self.emaspan = params["emaspan"]

    def factor_calc(self):
        # ---- 1. 请设定对应的因子原始值文件-------
        corresponding_non_factor_file = "NF_D_UpDownVolSub_10.h5"
        # ---- 第1部分到此为止，其他部分勿改动 ----
        ...
        # ---- 2. 对因子原始值求ema ----------------
        ans_df = factor_original_df.ewm(span=self.emaspan).mean()
        # ---- 第2部分到此为止，以下勿改动----------
