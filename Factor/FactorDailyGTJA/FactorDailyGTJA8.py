...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag_k):
    ...
    high_df = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date, pv_type='high',
                                        adj_type='FORWARD')
    low_df = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date, pv_type='low', adj_type='FORWARD')
    ...
    temp_df1 = (high_df + low_df).div(2).mul(0.2) + vwap_df.mul(0.8)
    temp_df2 = temp_df1.diff(lag_k).mul(-1)
    ans_df = temp_df2[alpha_universe == 1].rank(axis=1)
