...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, ma_k):
    ...
    data_df_close = Dtk.get_panel_daily_pv_df(stock_list, start_date, end_date_plus_k_2, pv_type='close',
                                              adj_type='FORWARD')
    data_df_low = Dtk.get_panel_daily_pv_df(stock_list, start_date, end_date_plus_k_2, pv_type='low',
                                            adj_type='FORWARD')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    data_rolling_low: pd.DataFrame = data_df_low.rolling(ma_k).min()
    tag_high = data_rolling_low.shift(-ma_k) / data_df_close - 1
