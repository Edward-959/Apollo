...
class FactorDailyMinSDVVOL(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            stock_minute_vol5 = stock_minute_vol.rolling(5, axis=1).sum()
            ...
            stock_minute_5vol = stock_minute_vol5[temp_min_list]
            stock_minute_VVOL = stock_minute_5vol.std(ddof=0, axis=1)

            stock_minute_cov1 = stock_minute_VVOL.ewm(halflife=self.__n).cov(stock_minute_VVOL.shift(1))
            stock_minute_cov2 = stock_minute_VVOL.ewm(halflife=self.__n).cov(stock_minute_VVOL.shift(2))
            # stock_minute_cov3 = stock_minute_VVOL.ewm(halflife=self.__n).cov(stock_minute_VVOL.shift(3))

            stock_StdEW = stock_minute_VVOL.ewm(halflife=self.__n).std() + 2 * (
