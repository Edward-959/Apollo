...
# @Time    : 2018/9/3 13:22
# @Author  : 011673
# @File    : FactorMinIntraCoppockCurve.py
...
class FactorMinIntraCC(MinFactorBase):
...
def test():
    ...
    factor_generator = FactorMinIntraCC(codes=[code], start_date_int=start,
                                         end_date_int=end, name=file_name, save_path=save_dir, lag=[9, 12, 26])
...
def main():
    ...
    factor_name = "F_M_CoppockCurve"+str([9,12,26])  # 这个因子名可以加各种后缀，用于和相近的因子做区分
    file_name = factor_name
    factor_generator = FactorMinIntraCC(codes=stock_code_list, start_date_int=istart_date_int,
