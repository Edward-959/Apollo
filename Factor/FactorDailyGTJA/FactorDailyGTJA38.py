# -*- coding: utf-8 -*-
# @Time    : 2018/11/15 15:54
# @Author  : 011673
# @File    : FactorDailyGTJA38.py
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, k):
    ...
    high_mean=high.rolling(window=20,min_periods=1).mean()
    # condition=(high_mean<high)
    delta_high=-high+high.shift(2)
    # delta_high[condition == False]=0
    factor_data=delta_high
...
def main():
    ...
    for i_lag in [3, 5]:
        k = i_lag
        factor_name = "F_D_GTJA38_shift_" + '_' + str(k)
