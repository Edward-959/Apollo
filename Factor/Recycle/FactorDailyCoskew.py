...
revised on 2019/01/24
@author: 006688
过去n天股票收益率与市场指数收益率的协偏度
mean((r-r.mean)*(r_m-r_m.mean)^2)/(std(r)*std(r_m)^2)
"""
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, n, index_code):
    ...
    stock_pct_chg = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_lag_2, end_date, pv_type='pct_chg')
    stock_volume = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_lag_2, end_date, pv_type='volume')
    index_close = Dtk.get_panel_daily_pv_df([index_code], start_date_minus_lag_2, end_date, pv_type='close')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    stock_pct_chg = stock_pct_chg * stock_volume / stock_volume
    ...
    for date in trading_days:
        ...
        index_pct_chg_i = index_pct_chg_i[index_code]
        stock_pct_chg_i = stock_pct_chg_i.sub(stock_pct_chg_i.mean())
        index_pct_chg_i = index_pct_chg_i.sub(index_pct_chg_i.mean())
        coskew_i = stock_pct_chg_i.apply(lambda x: (x * (index_pct_chg_i ** 2)).mean()/(x.std()*index_pct_chg_i.var()),
                                         raw=True)
        factor_data.loc[date] = coskew_i
    factor_data = factor_data * stock_volume / stock_volume
...
def main():
    ...
    for n in [20]:
        factor_name = "F_D_Coskew_" + str(n) + "_" + index_code[0:6]
        file_name = factor_name
        factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, n, index_code)
