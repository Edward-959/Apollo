...
@author: 006688
一致买入交易因子：上涨的实体K线成交量除以当日总成交量得到当日因子值，再求移动平均
"""
...
class FactorDailyMinPositiveConsistentVolume(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:  # 如可正常取到行情DataFrame
            ...
            consistent_bar = (stock_minute_close_5 - stock_minute_open_5) >= 0.95 * (stock_minute_high_5 - stock_minute_low_5)
...
def main():
    ...
    for i in [10]:
        factor_name = "F_D_MinPositiveConsistentVolume_" + str(i)   # 这个因子名可以加各种后缀，用于和相近的因子做区分
        file_name = factor_name
        factor_generator = FactorDailyMinPositiveConsistentVolume(codes=stock_code_list, start_date_int=istart_date_int,
