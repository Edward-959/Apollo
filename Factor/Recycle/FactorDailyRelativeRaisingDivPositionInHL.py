...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag, benchmark):
    ...
    data_index: pd.DataFrame = Dtk.get_panel_daily_pv_df([benchmark], start_date_minus_lag_2, end_date,
                                                         pv_type='close')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    stock_return_df = data_close / data_close.shift(lag) - 1
    index_return_df = data_index / data_index.shift(lag) - 1
    index_return_df = pd.DataFrame(np.tile(index_return_df.values, [1, stock_return_df.shape[1]]), index=stock_return_df.index, columns=stock_return_df.columns)
    excess_return_df = stock_return_df - index_return_df

    rolling_high_df = data_high.fillna(method='ffill').rolling(lag).max()
    rolling_low_df = data_low.fillna(method='ffill').rolling(lag).min()

    relation_position = (rolling_high_df - data_close) / (rolling_high_df - rolling_low_df)

    factor_data = excess_return_df * relation_position
...
def main():
    ...
    benchmark = '000905.SH'
    for lag in [6, 12, 18]:
        factor_name = "F_D_RelativeRaisingDivPositionInHL"+'Lag'+str(lag)+'_Benchmark'+benchmark
        file_name = factor_name
        factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, lag, benchmark)
