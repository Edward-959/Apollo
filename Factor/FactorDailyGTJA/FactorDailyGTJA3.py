...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag_k):
    ...
    ############################################
    start_date_minus_k_2 = Dtk.get_n_days_off(start_date, -(lag_k + 2))[0]
    ...
    close_df: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date, pv_type='close',
                                                       adj_type='FORWARD')
    low_df: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date, pv_type='low',
                                                     adj_type='FORWARD')
    high_df: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date, pv_type='high',
                                                      adj_type='FORWARD')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    ans_part1 = close_df.copy()
    ans_part1[close_df == close_df.shift(1)] = 0

    temp_df1 = high_df.copy()
    temp_df1[close_df.shift(1) > high_df] = close_df.shift(1).copy()

    temp_df2 = low_df.copy()
    temp_df2[close_df.shift(1) < low_df] = close_df.shift(1).copy()

    ans_part2 = temp_df1.copy()
    ans_part2[close_df > close_df.shift(1)] = temp_df2.copy()

    ans_df = ans_part1 - ans_part2
    ans_df = ans_df.fillna(method='ffill')
    ans_df = ans_df.rolling(window=lag_k).sum()
...
def main():
    ...
    factor_name = "F_D_GTJA3"
    file_name = factor_name
    factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, lag_k=6)
