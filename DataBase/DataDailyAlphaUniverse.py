...
def alpha_universe(date):
    ...
    # 建立五个字段，符合每个字段下每日符合条件则记为1，最后将5个字段的值相加，值为5的即为当日股票池成员
    columns = ['listing_1yr', 'NoSTPT', 'HalfYearInTrade', '95%MarketValue', 'NoSuspended']
    alpha_universe = pd.DataFrame(index=stock_list, columns=columns, data=0)
    ...
    for stock in stock_list:
        alpha_universe.at[stock, '95%MarketValue'] = 1
    ...
    for stock in alpha_universe.index.tolist():
        stock_index = stock_list.index(stock)
        if half_year_in_trade_bool[stock_index] + 0 == 1:
            alpha_universe.at[stock, 'HalfYearInTrade'] = 1
        else:
            alpha_universe.at[stock, 'HalfYearInTrade'] = 0
        ...
        if daydiff.days > 365:
            alpha_universe.at[stock, 'listing_1yr'] = 1
        else:
            alpha_universe.at[stock, 'listing_1yr'] = 0
    # 读第三张表
    ...
    for stock in alpha_universe.index.tolist():
        stock_index = stock_list.index(stock)
        if noSuspend_bool[stock_index] == 1.0:
            alpha_universe.at[stock, 'NoSuspended'] = 1
        else:
            alpha_universe.at[stock, 'NoSuspended'] = 0
        if noSTPT_bool[stock_index] == 1.0:
            alpha_universe.at[stock, 'NoSTPT'] = 1
        else:
            alpha_universe.at[stock, 'NoSTPT'] = 0
    alpha_universe_1 = []
    for stock in alpha_universe.index.tolist():
        if sum(alpha_universe.loc[stock]) == 5:
            alpha_universe_1.append(stock)
    return alpha_universe_1


class DataDailyAlphaUniverse(DailyFactorBase):
    ...
    def factor_calc(self):
        ...
        alpha_universe_df = pd.DataFrame(columns=stock_list, index=tradingDay, data=0)
        for day in tradingDay:
            alpha_universe_list = alpha_universe(day)
            for stock in alpha_universe_list:
                alpha_universe_df.at[day, stock] = 1
