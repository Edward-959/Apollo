...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag_k):
    ...
    open_df = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date, pv_type='open', adj_type='FORWARD')
    high_df = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date, pv_type='high', adj_type='FORWARD')
    ...
    temp_df1 = open_df.mul(0.85) + high_df.mul(0.15)
    temp_df2 = temp_df1.diff(4)
    sign_df = pd.DataFrame(np.sign(temp_df2.values), index=temp_df2.index, columns=temp_df2.columns)
    ans_df = sign_df[alpha_universe == 1].rank(axis=1)
    ans_df = ans_df.mul(-1)
