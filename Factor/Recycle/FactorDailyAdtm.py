...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag):
    ...
    data_open: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_lag_2, end_date,
                                                        pv_type='open', adj_type='FORWARD')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    data_diff_open = data_open - data_open.shift(1)
    data_dtm = np.maximum(data_high - data_open, abs(data_diff_open))
    data_dtm[data_diff_open <= 0] = 0
    data_dbm = np.maximum(data_open - data_low, abs(data_diff_open))
    data_dbm[data_diff_open >= 0] = 0
    data_stm = data_dtm.rolling(lag).sum()
    data_sbm = data_dbm.rolling(lag).sum()
    data_adtm = (data_stm - data_sbm) / np.maximum(data_stm, data_sbm)
    data_adtm[data_stm == data_sbm] = 0
    factor_data = data_adtm
