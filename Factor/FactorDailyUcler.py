...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, n):
    ...
    stock_close: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_lag_2, end_date,
                                                          pv_type='close', adj_type='FORWARD')
    stock_high: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_lag_2, end_date,
                                                         pv_type='high', adj_type='FORWARD')
    # stock_turn: pd.DataFrame = Dtk.get_panel_daily_info(stock_list, start_date_minus_lag_2, end_date, info_type='turn')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    stock_nday_high = stock_high.shift(1).rolling(n).max()
    factor_data = (stock_close - stock_nday_high)/stock_nday_high
    factor_data = factor_data.ewm(span=5).mean()
    # factor_data1 = factor_data.rolling(5).mean()
...
def main():
    ...
    for n in [5]:
        factor_name = "F_D_UclerEwm" + str(n)
