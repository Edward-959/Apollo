...
class NonFactorDailyMinVolCorr(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            stock_vol_corr = stock_minute_volume.corrwith(stock_minute_volume.shift(1, axis=1), axis=1)
            factor_data = stock_vol_corr * self.__data_volume.loc[:, code] / self.__data_volume.loc[:, code]
