...
"""
@author: 006688
平均真实波幅(ATR)是显示市场变化率的指标。ATR不指示价格的变动方向，只表示价格的波动程度。
而价格波动幅度的突破通常也预示着价格的突破。即：该指标价值越高，趋势改变的可能性就越高；该指标的价值越低，趋势的移动性就越弱。
...
class FactorMinIntraAtr(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            ...
            stock_minute_last_close = stock_minute_close.shift(1, axis=1)
            stock_minute_tr = np.maximum(stock_minute_high - stock_minute_low, np.maximum(
                abs(stock_minute_high - stock_minute_last_close), abs(stock_minute_low - stock_minute_last_close)))
            stock_minute_atr = stock_minute_tr.rolling(self.__lag, min_periods=1, axis=1).mean() / stock_minute_close
            stock_minute_data[code] = stock_minute_atr.stack()
...
def main():
    ...
    for lag in [14]:
        factor_name = "F_M_IntraAtr" + str(lag)  # 这个因子名可以加各种后缀，用于和相近的因子做区分
        file_name = factor_name
        factor_generator = FactorMinIntraAtr(codes=stock_code_list, start_date_int=istart_date_int,
