...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag_k1, lag_k2):
    ...
    ############################################
    start_date_minus_k_2 = Dtk.get_n_days_off(start_date, -(lag_k1 + 2))[0]
    ...
    # Rolling_rank 排序函数来自 https://github.com/pandas-dev/pandas/issues/9481
    # 此函数支持升序和降序、也支持当遇到相同值时返回的是min, avg还是max
    # 缺点是比较慢
    def rolling_rank_on_series(array):
        a = array.rank(method='min', ascending=False)
        b = a.iloc[len(array) - 1]
        return b

    tsrank_vol = vol_df.rolling(lag_k1).apply(rolling_rank_on_series, raw=False)
    tsrank_high = high_df.rolling(lag_k1).apply(rolling_rank_on_series, raw=False)
    tsrank_vol = tsrank_vol.fillna(method='ffill')
    tsrank_high = tsrank_high.fillna(method='ffill')
    corr_df = tsrank_vol.rolling(lag_k1).corr(tsrank_high)
    ans_df = corr_df.rolling(lag_k2).max()
    ans_df = ans_df * -1
...
def main():
    ...
    for lag1, lag2 in zip([5], [3]):
        factor_name = "F_D_GTJA5"
