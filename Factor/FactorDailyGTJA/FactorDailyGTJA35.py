# -*- coding: utf-8 -*-
# @Time    : 2018/11/15 14:17
# @Author  : 011673
# @File    : FactorDailyGTJA35.py
# -*- coding: utf-8 -*-
# @Time    : 2018/11/15 14:14
# @Author  : 011673
# @File    : FactorDailyGTJA34.py
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, k):
    ...
    open_delta = open_ - open_.shift(1)
    signal_1: pd.DataFrame = decay_linear(open_delta, 15)
    cor = volume.rolling(window=17, min_periods=1).corr(open_)
    dl_2: pd.DataFrame = decay_linear(cor, 7)
    rank_2 = dl_2.rank(axis=1)
    value = np.minimum(signal_1.values, rank_2.values)
    value = pd.DataFrame(value, index=rank_2.index, columns=rank_2.columns)
    factor_data = -value
...
def main():
    ...
    for i_lag in [3, 5]:
        k = i_lag
        factor_name = "F_D_GTJA35" + '_' + str(k)
