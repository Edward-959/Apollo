...
"""
@author: 006688
计算过去m日的日均换手率的标准差除以最近n（m<n）日内日均换手率的标准差，再减去1
"""
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, m, n):
    ...
    # 计算因子值
    factor_data = stock_turn.rolling(m).std() / stock_turn.rolling(n).std() - 1
...
def main():
    ...
    for m, n in [(20, 250)]:
        factor_name = "F_D_TurnStdBias_" + str(m) + "_" + str(n)
        file_name = factor_name
        factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, m, n)
