...
Style: Downside Risk
Lower Partial Moment: 𝐿𝑃𝑀(𝑡)=1/|𝐼| * Σ(𝑟(𝑖)−𝑟𝑞)^2
"""
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name):
    ...
    l = 42
    tau = 252
    start_date_minus_lag_2 = Dtk.get_n_days_off(start_date, -l-tau-2)[0]
    ...
    # 计算因子值
    stock_return = stock_close.diff() / stock_close.shift(1)
    trading_days = Dtk.get_trading_day(start_date, end_date)
    stock_lpm = pd.DataFrame(index=trading_days, columns=stock_return.columns)
    for date in trading_days:
        i = stock_return.index.tolist().index(date)
        temp_return = stock_return.iloc[i-l-tau+1:i-l+1]
        r_q = np.quantile(temp_return, q, axis=0)
        temp_return = temp_return.sub(r_q, axis=1)
        stock_lpm.loc[date] = np.sum(np.square(temp_return[temp_return < 0]), axis=0) / (temp_return < 0).sum(axis=0)
    factor_data = stock_lpm.astype(float)
