...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag):
    ...
    # 计算因子值
    data_high_id = data_high.rolling(lag).apply(lambda x: Series(x).idxmax(), raw=True)
    data_low_id = data_low.rolling(lag).apply(lambda x: Series(x).idxmin(), raw=True)
    data_aroon_up = data_high_id / lag
    data_aroon_down = data_low_id / lag
    data_aroon = data_aroon_up - data_aroon_down
    factor_data = data_aroon
