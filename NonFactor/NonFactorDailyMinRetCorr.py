...
class NonFactorDailyMinRetCorr(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            stock_minute_chg = stock_minute_close / stock_minute_close.shift(1, axis=1) - 1
            stock_ret_corr = stock_minute_chg.corrwith(stock_minute_chg.shift(1, axis=1), axis=1)
            factor_data = stock_ret_corr * self.__data_volume.loc[:, code] / self.__data_volume.loc[:, code]
