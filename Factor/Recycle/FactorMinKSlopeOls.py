...
#     y = y / y[0]
#     x = range(1, len(y) + 1)
#     return lsfit(error, x, y)[0]


def kfit(y):
    y = y / y[0]
    x = range(1, len(y) + 1)
    k = sum((x - np.mean(x)) * (y - np.mean(y))) / sum(np.square(x - np.mean(x)))
    return k


class FactorMinKSlopeOls(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        n = self.__lag
        # round_digit = min(round(n * 0.2), int(n / 2))
        close = stock_minute_data['close'].to_frame()
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            close_kslope = close.rolling(window=n).apply(lambda x: kfit(x), raw=False)
            close_kslope.columns = [code]
            factor_data = close_kslope
...
def main():
    ...
    ###############################################
    # i_start_date_int = 20130701
    i_start_date_int = 20170601
    i_end_date_int = 20180630
    for lag in [30, 60,  90, 130]:
        factor_name = "F_M_KSlopeOls" + str(lag)  # 这个因子名可以加各种后缀，用于和相近的因子做区分
        file_name = factor_name
        factor_generator = FactorMinKSlopeOls(codes=stock_code_list, start_date_int=i_start_date_int,
