"""
Created on 2018/8/30 by 006566 范例，分钟级因子+带参数
"""
...
class FactorMinGrowthK(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        ############################################
        effective_start_date = Dtk.get_n_days_off(start, -1 * self.__k)[0]
        stock_minute_data = Dtk.get_single_stock_minute_data(code, effective_start_date, end, fill_nan=True,
        ...
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            ...
            stock_minute_data[code] = stock_minute_data['close'] / stock_minute_data['close'].shift(self.__k) - 1
...
def main():
    ...
    ###############################################
    # 以下4行及factor_generator的类名需要自行改写 #
    ###############################################
    istart_date_int = 20180601
    iend_date_int = 20180630
    k = 180
    factor_name = "F_M_Growth" + str(k)  # 这个因子名可以加各种后缀，用于和相近的因子做区分
    file_name = factor_name
    factor_generator = FactorMinGrowthK(codes=stock_code_list, start_date_int=istart_date_int,
