...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name):
    ...
    # 计算因子值
    temp_df1 = (high_df + low_df) / 2
    temp_df2 = (high_df.shift(1) + low_df.shift(1)) / 2
    temp_df3 = (temp_df1 + temp_df2) * (high_df - low_df).div(vol_df)
    ans_df = temp_df3.ewm(span = n-1).mean()
