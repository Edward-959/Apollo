...
"""
@author: 006688
价量趋势（price and volume trend）因子
原始计算方式为：x=(当前K线收盘价—前一根K线收盘价)/前一根K线收盘价×当前K线成交量,PVT指标值为前lag个x值的累加
这里适当进行了更改：x=(当前K线收盘价—当前K线开盘价)/当前K线开盘价×当前K线成交额
"""
...
class FactorMinIntraPVT(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            stock_minute_pvt = (stock_minute_data['close'] / stock_minute_data['open'] - 1) * stock_minute_data['amt']
            stock_minute_pvt = stock_minute_pvt.unstack()
            stock_minute_pvt = stock_minute_pvt.rolling(self.__lag, min_periods=1, axis=1).sum()
            stock_minute_data[code] = stock_minute_pvt.stack()
...
def main():
    ...
    for lag in [242]:
        factor_name = "F_M_IntraPVT" + str(lag)  # 这个因子名可以加各种后缀，用于和相近的因子做区分
        file_name = factor_name
        factor_generator = FactorMinIntraPVT(codes=stock_code_list, start_date_int=istart_date_int,
