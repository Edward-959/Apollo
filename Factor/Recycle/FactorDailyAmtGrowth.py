...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, n):
    ...
    ############################################
    start_date_minus_n_2 = Dtk.get_n_days_off(start_date, -(n+2))[0]
    data_df: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_n_2, end_date,
                                                      pv_type='amt', adj_type='NONE')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    factor_data = data_df / data_df.rolling(n).mean() - 1
...
def main():
    ...
    for n in [3, 5, 10, 20]:
        factor_name = "F_D_AmtGrowth" + '_' + str(n)
