...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag_k):
    ...
    close_df: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date, pv_type='close',
                                                       adj_type='FORWARD')
    open_df: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date, pv_type='open',
                                                      adj_type='FORWARD')
    ...
    log_vol_df = np.log(vol_df)
    delta_log_vol_df = log_vol_df.diff()
    delta_log_vol_rank_df = delta_log_vol_df[alpha_universe == 1].rank(axis=1)
    delta_log_vol_rank_df = delta_log_vol_rank_df.fillna(method='ffill')
    intraday_return = (close_df - open_df) / open_df
    intraday_return = intraday_return.fillna(method='ffill')
    intraday_return_rank_df = intraday_return[alpha_universe == 1].rank(axis=1)
    corr_df = delta_log_vol_rank_df.rolling(window=lag_k).corr(intraday_return_rank_df)
    corr_df = corr_df * -1
    factor_data = corr_df.loc[start_date: end_date].copy()
    ########################################
    # 因子计算逻辑到此为止，以下勿随意变更 #
    ########################################
...
def main():
    ...
    for lag in [6]:
        factor_name = "F_D_GTJA1"
        file_name = factor_name
        factor_generator(stock_code_list, istart_date_int, iend_date_int, save_dir, file_name, lag)
