...
class FactorMinIntraDistToAPMA(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            stock_minute_avgprice = stock_minute_data['amt'] / stock_minute_data['volume']
            stock_minute_apma = stock_minute_avgprice.unstack()
            stock_minute_apma = stock_minute_apma.rolling(self.__lag, min_periods=1, axis=1).mean()
            stock_minute_data[code] = stock_minute_data['close'] / stock_minute_apma.stack() - 1
...
def main():
    ...
    for lag in [10]:
        factor_name = "F_M_IntraDistToAPMA" + str(lag)  # 这个因子名可以加各种后缀，用于和相近的因子做区分
        file_name = factor_name
        factor_generator = FactorMinIntraDistToAPMA(codes=stock_code_list, start_date_int=istart_date_int,
