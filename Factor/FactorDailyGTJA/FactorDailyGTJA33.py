# -*- coding: utf-8 -*-
# @Time    : 2018/11/15 13:18
# @Author  : 011673
# @File    : FactorDailyGTJA33.py
# -*- coding: utf-8 -*-
...
def ts_rank(df: pd.DataFrame, n: int):
    def find_value(data):
        data = list(data)
        last = data[-1]
        data.sort()
        return data.index(last) + 1
    return df.rolling(window=n, min_periods=1).apply(find_value)
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, k):
    ...
    low: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date,
    ...
    low_5_min = low.rolling(window=5).min()
    signal1 = low_5_min.shift(5) - low_5_min
    signal2: pd.DataFrame = (ret.rolling(window=240, min_periods=1).sum() - ret.rolling(window=20,
                                                                                        min_periods=1).sum()) / 220
    signal2 = signal2.rank(axis=1)
    signal3 = ts_rank(volume, 5)
    factor_data = signal1 * signal2 * signal3
...
def main():
    ...
    for i_lag in [3, 5]:
        k = i_lag
        factor_name = "F_D_GTJA33" + '_' + str(k)
