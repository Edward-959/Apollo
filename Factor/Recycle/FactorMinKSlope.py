...
class FactorMinKSlope(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            close_mean = close.rolling(window=round_digit).mean()
            result = abs(close_mean / close_mean.shift(n-round_digit) - 1)
            result.columns = [code]
            factor_data = result
...
def main():
    ...
    for lag in [20]:
        factor_name = "F_M_Slope" + str(lag)  # 这个因子名可以加各种后缀，用于和相近的因子做区分
        file_name = factor_name
        factor_generator = FactorMinKSlope(codes=stock_code_list, start_date_int=i_start_date_int,
