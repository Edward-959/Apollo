...
"""
@author: 006566
计算截断时刻收盘价格在观察期（包括当日）内最高点和最低点之间的相对位置，取值为0~1
"""
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, k):
    ...
    data_high_cut: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date,
                                                            pv_type='high_cut', adj_type='FORWARD')
    data_low: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date,
                                                       pv_type='low', adj_type='FORWARD')
    data_low_cut: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_k_2, end_date,
                                                           pv_type='low_cut', adj_type='FORWARD')
    # 计算因子值
    if k > 1:
        data_high_k = np.maximum(data_high.shift(1).rolling(k-1).max(), data_high_cut)
        data_low_k = np.minimum(data_low.shift(1).rolling(k-1).min(), data_low_cut)
    else:
        data_high_k = data_high_cut
        data_low_k = data_low_cut
    factor_data = (data_close_cut - data_low_k) / (data_high_k - data_low_k)
...
def main():
    ...
    for i_lag in [3, 5]:
        k = i_lag
        factor_name = "F_D_CutPositionHL" + '_' + str(k)
