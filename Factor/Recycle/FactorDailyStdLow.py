...
"""
@author: 006688
用个股每日最低价除以前一日收盘价计算日内最大跌幅，此处有复权处理，再计算最大跌幅序列的标准差
"""
...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, n):
    ...
    stock_low: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_lag_2, end_date,
                                                        pv_type='low', adj_type='FORWARD')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    stock_return_low = (stock_low / stock_close.shift(1) - 1) * 100
    factor_data = stock_return_low.rolling(n).std()
