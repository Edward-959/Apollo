...
class NonFactorDailyMinAbnAmtRet(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            stock_minute_amt_temp = stock_minute_amt.iloc[:, 1:-3]
            abn = stock_minute_amt_temp.mean(axis=1) + stock_minute_amt_temp.std(axis=1)*0.66
            stock_minute_amt = stock_minute_amt.iloc[:, 1:-1]
            factor_data = (stock_minute_amt.T - abn).T
            factor_data = factor_data > 0
            ret = stock_minute_data['close'].unstack()
            ret = ret / ret.shift(-1, axis=1) - 1
            ret: pd.DataFrame = 100 * ret
            ret = ret.loc[:, stock_minute_amt.columns]
            ret[~factor_data] = np.nan
            factor_data = ret.sum(axis=1)
            ava_data = stock_minute_amt_temp.sum(axis=1)
