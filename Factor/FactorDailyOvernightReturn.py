...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, n):
    ...
    stock_pre_close: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_lag_2, end_date,
                                                              pv_type='pre_close', adj_type='NONE')
    stock_open: pd.DataFrame = Dtk.get_panel_daily_pv_df(stock_list, start_date_minus_lag_2, end_date,
                                                         pv_type='open', adj_type='NONE')
    t2 = dt.datetime.now()
    print("Raw data was downloaded successfully and it cost", t2 - t1)
    # 计算因子值
    stock_overnight_return = (stock_open / stock_pre_close - 1) * 100
    factor_data = stock_overnight_return.rolling(n).sum()
...
def main():
    ...
    for n in [15]:
        factor_name = "F_D_OvernightReturn_" + str(n)
