...
def factor_generator(stock_list, start_date, end_date, factor_file_dir_path, factor_file_name, lag):
    ...
    # 计算因子值
    temp_value1 = (data_close - data_close.shift(lag)) / data_close.shift(lag)
    temp_value2 = (data_close - data_close.shift(lag)) / data_close
    factor_data = temp_value2.copy()
    cond1 = data_close < data_close.shift(lag)
    factor_data[cond1] = temp_value1[cond1]
...
def main():
    ...
    for lag in [5, 10, 15]:
        factor_name = "F_D_GTJA19"+'Lag'+str(lag)
