...
@author: 006547
Modified by 006566 on 2018/11/26
"""
...
class FactorDailyMin1430TurnoverDirection(MinFactorBase):
    def __init__(self, codes: List[str] = ..., start_date_int: int = ..., end_date_int: int = ...,
        ...
        # 所有要用到的日级别信息，应在此获取
        data_free_float_shares = Dtk.get_panel_daily_info(codes, start_date_int, end_date_int, 'free_float_shares')
        self.__data_free_float_shares = data_free_float_shares.multiply(10000)
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:  # 如可正常取到行情DataFrame
            ...
            stock_minute_volume_select = stock_minute_volume.loc[:, 1430:]
            stock_minute_volume_select = stock_minute_volume_select.sum(axis=1, skipna=False)
            data_free_float_shares = self.__data_free_float_shares[code]
            stock_turnover = stock_minute_volume_select / data_free_float_shares
            stock_minute_close = stock_minute_data['close'].unstack()
            direction = stock_minute_close.loc[:, 1500] / stock_minute_close.loc[:, 1430] - 1
            direction[direction > 0] = 1
            direction[direction < 0] = -1
            stock_turnover_direction = stock_turnover * direction
            factor_data = pd.DataFrame(stock_turnover_direction, index=list(stock_minute_close.index), columns=[code])
        ########################################
        # 因子计算逻辑到此为止，以下勿随意变更 #
        ########################################
...
def main():
    ...
    factor_name = "F_D_Min1430TurnoverDirection"   # 这个因子名可以加各种后缀，用于和相近的因子做区分
    file_name = factor_name
    factor_generator = FactorDailyMin1430TurnoverDirection(codes=stock_code_list, start_date_int=istart_date_int,
