...
class FactorMinKSwing(MinFactorBase):
    ...
    def single_stock_factor_generator(self, code: str = ..., start: int = ..., end: int = ...):
        ...
        if stock_minute_data.columns.__len__() > 0:  # 如可正常取到行情DataFrame
            stock_minute_high = stock_minute_data['close'].rolling(self.__lag).max()
            stock_minute_low = stock_minute_data['close'].rolling(self.__lag).min()
            stock_minute_data[code] = (stock_minute_high - stock_minute_low) / stock_minute_low
...
def main():
    ...
    iend_date_int = 20160630
    for lag in [20]:
        factor_name = "F_M_MinKSwing" + str(lag)  # 这个因子名可以加各种后缀，用于和相近的因子做区分
        file_name = factor_name
        factor_generator = FactorMinKSwing(codes=stock_code_list, start_date_int=istart_date_int,
