...
class NonFactorDailyMin_early_close_ret(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            time_list = []
            for time in close_min.columns:
                if 925 < time < 1030:
                    time_list.append(time)
            close_min = close_min.loc[:, time_list]
            factor_data = close_min.iloc[:, -1] / close_min.iloc[:, 0]
