...
class NonFactorDailyMinMidPeriodAmtRatio(DailyMinFactorBase):
    ...
    def single_stock_factor_generator(self, code):
        ...
        if stock_minute_data.columns.__len__() > 0 and data_check_df.__len__() > 0:
            ...
            mid__amt = stock_minute_amt.loc[:, stock_minute_amt.columns[
                (stock_minute_amt.columns <= 1430) & (stock_minute_amt.columns >= 1000)]]
            factor_data = (mid__amt.T - abn).T
